...
async function saveResponse(result){ const status=document.getElementById("save-status"); status.className="status";status.textContent="Saving the information you agreed to share…"; try{ const response=await fetch("/api/sessions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:state.sessionId,appVersion:APP_VERSION,consent:true,dogSex:state.dogSex,behaviorAnswers:state.behaviorAnswers,healthDurations:state.healthDurations,dogDemographics:state.dogDemographics,classKey})}); if(!response.ok)throw new Error(); const saved=await response.json(); if(saved.result.total!==result.total||saved.result.band!==result.band)throw new Error();status.textContent="Thank you—the information you agreed to share was saved for research.";}catch{status.classList.add("warning");status.textContent="Your result was calculated, but the research copy could not be saved. No action is required from you.";} }

async function saveClassFeedback(event){event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const feedback={enjoyment:Number(data.get("enjoyment")),clarity:Number(data.get("clarity")),resultUsefulness:Number(data.get("resultUsefulness"))};const error=document.getElementById("feedback-error");if(Object.values(feedback).some((value)=>!Number.isInteger(value)||value<1||value>5)){error.classList.remove("hidden");return;}error.classList.add("hidden");const button=document.getElementById("feedback-submit");button.disabled=true;button.textContent="Saving feedback…";try{const payload={submissionId:state.feedbackId,classKey,studentName:state.studentName,dogName:state.dogName,dogSex:state.dogSex,behaviorAnswers:state.behaviorAnswers,healthDurations:state.healthDurations,dogDemographics:state.dogDemographics,...feedback};const response=await fetch("/api/class-submissions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});if(!response.ok)throw new Error();await response.json();form.classList.add("hidden");const confirmation=document.getElementById("feedback-confirmation");confirmation.textContent="Thank you—your questionnaire answers and class feedback were saved for the class exercise.";confirmation.classList.remove("hidden");}catch{error.textContent="The class response could not be saved. Please try again.";error.classList.remove("hidden");button.disabled=false;button.textContent="Submit feedback →";}}

async function finish(){ const result=calculateDslq(state.dogSex,state.behaviorAnswers,state.healthDurations);renderResult(result);document.getElementById("save-status").classList.add("hidden");document.getElementById("class-feedback").classList.toggle("hidden",classKey!=="drudell");document.getElementById("feedback-confirmation").classList.add("hidden");showScreen("result-screen");const tasks=[];if(classKey!=="drudell"&&state.consent)tasks.push(saveResponse(result));await Promise.allSettled(tasks); }

document.getElementById("start-button").addEventListener("click",()=>showScreen(classKey==="drudell"?"class-info-screen":"sex-screen"));
document.getElementById("class-info-form").addEventListener("submit",(event)=>{event.preventDefault();state.studentName=document.getElementById("student-name").value.trim();state.dogName=document.getElementById("class-dog-name").value.trim();document.getElementById("dog-name").value=state.dogName;if(state.studentName)showScreen("sex-screen");});
...
document.getElementById("health-form").addEventListener("submit",(event)=>{event.preventDefault();if(!readHealth()){document.getElementById("health-error").classList.remove("hidden");return;}if(state.healthIndex<healthItems().length-1){state.healthIndex++;renderHealth();}else showScreen(classKey==="drudell"?"demographics-screen":"consent-screen");});
...
document.getElementById("demographics-back").addEventListener("click",()=>{if(classKey==="drudell"){state.healthIndex=healthItems().length-1;renderHealth();showScreen("health-screen");}else showScreen("consent-screen");});
