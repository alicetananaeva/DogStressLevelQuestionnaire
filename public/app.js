...
const state = { sessionId: crypto.randomUUID(), feedbackId: crypto.randomUUID(), dogSex: "", behaviorAnswers: {}, behaviorIndex: 0, healthDurations: {}, healthIndex: 0, consent: false, dogDemographics: {} };
const classKey = new URLSearchParams(window.location.search).get("class");
if(classKey==="drudell"){document.getElementById("class-pilot-note").classList.remove("hidden");document.getElementById("class-consent-note").classList.remove("hidden");}
...
async function saveClassFeedback(event){event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const feedback={enjoyment:Number(data.get("enjoyment")),clarity:Number(data.get("clarity")),resultUsefulness:Number(data.get("resultUsefulness"))};const error=document.getElementById("feedback-error");if(Object.values(feedback).some((value)=>!Number.isInteger(value)||value<1||value>5)){error.classList.remove("hidden");return;}error.classList.add("hidden");const button=document.getElementById("feedback-submit");button.disabled=true;button.textContent="Saving feedback…";try{const payload={feedbackId:state.feedbackId,classKey,consent:state.consent,...feedback};if(state.consent){Object.assign(payload,{sessionId:state.sessionId,dogSex:state.dogSex,behaviorAnswers:state.behaviorAnswers,healthDurations:state.healthDurations,dogDemographics:state.dogDemographics});}const response=await fetch("/api/pilot",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});if(!response.ok)throw new Error();const saved=await response.json();form.classList.add("hidden");document.getElementById("completion-code").textContent=saved.code;document.getElementById("completion-card").classList.remove("hidden");if(state.consent){const status=document.getElementById("save-status");status.className="status";status.textContent="Thank you—your questionnaire answers and class feedback were saved.";}}catch{error.textContent="The feedback could not be saved. Please try again.";error.classList.remove("hidden");button.disabled=false;button.textContent="Submit feedback and get code →";}}

async function finish(){ const result=calculateDslq(state.dogSex,state.behaviorAnswers,state.healthDurations);renderResult(result);document.getElementById("save-status").classList.add("hidden");document.getElementById("class-feedback").classList.toggle("hidden",classKey!=="drudell");document.getElementById("completion-card").classList.add("hidden");showScreen("result-screen");const tasks=[];if(classKey!=="drudell"&&state.consent)tasks.push(saveResponse(result));await Promise.allSettled(tasks); }
