...
function healthResultHtml(){ const groups={3:[],2:[],1:[]}; for(const [code,duration] of Object.entries(state.healthDurations)){ if(duration!==-1&&!groups[duration].includes(HEALTH_LABELS[code])) groups[duration].push(HEALTH_LABELS[code]); } if(!Object.values(groups).some((values)=>values.length)) return ""; let html="<p>Some health issues you mentioned may be associated with chronic stress: they can both reflect it and contribute to it.</p>"; if(groups[3].length)html+=`<p><strong>Present for over a month:</strong> ${groups[3].join(", ")}. These signs have been present for a relatively long time. We recommend reviewing them with your veterinarian and a behavior or welfare professional.</p>`; if(groups[2].length)html+=`<p><strong>First observed less than a month ago:</strong> ${groups[2].join(", ")}. If your veterinarian has not identified a medical cause, a consultation with a behavior or welfare professional may also be helpful.</p>`; if(groups[1].length)html+=`<p><strong>First noticed within the past week, or varying in timing:</strong> ${groups[1].join(", ")}. If these signs are persistent or worsening, a veterinary check may be warranted.</p>`; return html; }
